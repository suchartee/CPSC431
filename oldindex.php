  ...
  if ((isset($_POST["username"]) && !empty($_POST["username"])) && (isset($_POST["password"]) && (!empty($_POST["password"])))) {
    ...
    if ($stmt = $db->prepare($query)) {
      ...
      if($stmt->num_rows > 0) {
        ...
        // username and password are not empty and isset, but still need to wait
        ...
        echo '<script type="text/javascript"> alert("There is something wrong! Please try again.")</script>';
    }
  } else {
        if ($username == 'admin') {
            echo '<script type="text/javascript"> alert("You are tricky! Do not do this again.") </script>';
